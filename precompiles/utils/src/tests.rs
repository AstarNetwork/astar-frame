...
#[test]
fn read_addres_id_32() {
    let data = b"0000000000000000000000000000000*";

    let writer_output = EvmDataWriter::new().write(Bytes::from(&data[..])).build();

    let mut reader = EvmDataReader::new(&writer_output);
    let parsed: AccountId32 = reader.read().expect("to correctly parse AccountId32");
...
#[test]
fn write_address_id_32() {
    let data = b"0000000000000000000000000000000*";

    let writer_output = EvmDataWriter::new().write(Bytes::from(&data[..])).build();
    let mut reader = EvmDataReader::new(&writer_output);

    let expected_value = U256::from_big_endian(&data[..]);

    assert_eq!(reader.read::<U256>().expect("read offset"), 32_u64.into());
    assert_eq!(
        reader.read::<U256>().expect("read byte array size"),
        (&data[..]).len().into()
    );
    assert_eq!(
        reader.read::<U256>().expect("read address Id"),
        expected_value
    );
}
