...
fn non_vested_cannot_vest_other() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            assert!(!<VestingStorage<Test>>::contains_key(4));
            assert_noop!(
                Vesting::vest_other(Some(3).into(), 4),
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let transferred_amount =
                <Test as Config>::MinVestedTransfer::get() * max_schedules as u64;
...
fn build_genesis_has_storage_version_v1() {
    ExtBuilder::default()
        ...
        .build()
        .execute_with(|| {
            assert_eq!(super::pallet::StorageVersion::<Test>::get(), Releases::V1);
        });
}
