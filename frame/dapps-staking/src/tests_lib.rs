...
#[test]
fn staker_info_unregistered_era_adjust_no_more_claimables() {
    let mut staker_info = StakerInfo::<Balance>::default();

    let unregistered_era = 15;
    for offset in 0..=1 {
        assert_ok!(staker_info.stake(unregistered_era + offset, 1000));
        assert_eq!(staker_info.len(), 1);
...
#[test]
fn staker_info_unregistered_era_adjust_remaining_claimables() {
    let mut staker_info = StakerInfo::<Balance>::default();

    // Prepare scenario where we have some stake values prior to unregistered era and after
    let unregistered_era = 15;
    let eras_and_stakes = vec![
        (unregistered_era - 5, 100),
        (unregistered_era - 3, 157),
        (unregistered_era, 23),
        (unregistered_era + 3, 31),
    ];
    for (era, value) in &eras_and_stakes {
        assert_ok!(staker_info.stake(*era, *value));
    }

    // All eras that are equal or greater than unregistered era should be removed after adjustement
    assert_eq!(staker_info.len(), eras_and_stakes.len() as u32);
    staker_info.unregistered_era_adjust(unregistered_era);
    assert_eq!(staker_info.len(), 3);

    // After adjustement, latest staked value must be zero
    assert!(staker_info.latest_staked_value().is_zero());

    // All ears prior to the unregistered era must still be claimable
    for era in 10..unregistered_era {
        let (claim_era, staked) = staker_info.claim();
        assert_eq!(claim_era, era);
        assert!(!staked.is_zero());
    }

    // Further claim operations should return (0, 0)
    assert_eq!(staker_info.claim(), (0, 0));
}

#[test]
fn staker_info_unregistered_era_adjust_empty() {
    let mut staker_info = StakerInfo::<Balance>::default();
    staker_info.unregistered_era_adjust(10);
    assert!(staker_info.is_empty());
}
